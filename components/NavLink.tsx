...
function NavLink({ name, dropDownLinks, href, mobile }: Props) {
    return (
        <div className="flex justify-center group w-max">
            <div className={`dropdown relative`}>
                ...
                {/*  */}
                <ul className={`-translate-y-2 dropdown-menu w-36 absolute bg-background-light dark:bg-background-dark ${mobile ? 'bg-none shadow-none' : ''} text-base z-50 float-left py-2 list-none text-left rounded-lg shadow-lg mt-1 ${!mobile ? 'hidden group-hover:flex-col group-hover:flex' : 'flex top-1 left-24 '} m-0 bg-clip-padding border-none group-hover:bg-gradient-to-r group-hover:from-primary/20 to-secondary/50`}>
